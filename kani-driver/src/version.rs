...
///
/// There are three call sites (`kani-driver/src/main.rs`):
///
/// - The two call sites reached during a normal verification run gate this
///   behind `!args.common_args.quiet`, so this -- including the CBMC pin
///   mismatch warning below -- produces no output at all under `--quiet`.
///   That is a deliberate choice, not an oversight: `--quiet` has an
///   existing, tested contract that it produces *zero* output
///   (`tests/script-based-pre/check-quiet/check-quiet.sh` asserts `wc -l` is
///   0). A quiet, unattended measurement campaign is exactly the scenario the
///   CBMC pin check exists to protect -- and exactly where a user is least
///   likely to notice a warning even if we did print one -- but violating an
///   existing, enforced output contract to fix that is the wrong trade: it
///   would surprise every other quiet consumer for a benefit that a warning
///   buried in unread quiet output mostly wouldn't deliver anyway.
/// - The third call site handles `--version`/`-V` (see `StandaloneArgs` and
///   `CargoKaniArgs` in `kani-driver/src/args/mod.rs`, both of which set
///   `disable_version_flag = true` and define their own `version` field so
///   this function actually runs). This one is *not* gated on `--quiet`,
///   since it's an explicit, one-shot query rather than a verification run.
///   Anyone running a quiet campaign that cares about the CBMC pin should
///   run `kani --version` once before kicking it off -- and, unlike a
///   stale doc comment's worth of advice, that command genuinely reaches
///   [`print_cbmc_version_info`] rather than being intercepted by clap's
///   built-in version flag before this function is ever called.
...
fn print_cbmc_version_info() {
    ...
    if let Some(pinned) = pinned_cbmc_version() {
        if let Some(warning) = cbmc_version_mismatch_warning(&found, &pinned) {
